...
export class AutomationScheduler {
  ...
  async executeEventRun(
    ...
    // Guard: no concurrent event runs (shared workspace would corrupt git state)
    if (this.isRunning(autoId)) {
      console.warn(`[scheduler] Skipping event run for ${autoId}: already running`);
      const existing = [...this.activeRuns.values()].find((a) => a.run.automationId === autoId)!;
      return existing.run;
    }
  ...
  private async completeRun(
    ...
    if (auto.action.postResultAsComment && triggerEvent && status === "success") {
      ...
      if (ghRepo) {
        ...
        let body = fullContent ?? summary ?? "_No output_";
        if (body.length > MAX_COMMENT_CHARS) {
          body = body.slice(0, MAX_COMMENT_CHARS) + "\n\n…[truncated]";
        }
        const comment = `### Hive: ${auto.name}\n\n${body}\n\n---\n*Automated review generated by AI — not a human approval.*`;
        try {
