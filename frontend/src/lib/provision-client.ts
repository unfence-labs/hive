...
export interface ProvisionClient {
   ...
   * the app, complete forwards it to the CLI and returns the OAuth token.
   */
  startClaudeAuth(): Promise<{ url?: string }>;
  completeClaudeAuth(code: string): Promise<{ token: string }>;
  /**
   * Poll the running sign-in: the localhost-callback flow finishes without a
   * code, so the app watches for the token (or CLI exit) in the background.
   * `detail` carries the CLI's output tail when it exited token-less.
   */
...
export function ndjsonToEvent(o: NdjsonLine): ProvisionEvent | null {
  ...
  if (o.event === "run_end") {
    return {
      kind: "run_end",
      seq: o.seq,
      status: (o as { status?: string }).status === "error" ? "error" : "ok",
...
export function createTauriProvisionClient(): ProvisionClient {
  ...
  return {
    ...
    async completeClaudeAuth(code) {
      return invoke<{ token: string }>("claude_auth_code", { code });
    },
...
export function createMockProvisionClient(
  ...
  return {
    ...
    async completeClaudeAuth() {
      return { token: "sk-ant-oat01-mock" };
    },
    async pollClaudeAuth() {
