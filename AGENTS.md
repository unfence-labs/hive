...
- `backend/src/api/brain.ts`: singleton Brain routes — state read-create-connect-delete (`GET/POST/DELETE /api/brain`), working-tree file ops (`GET /api/brain/files`, `GET/PUT /api/brain/file`), and git ops (`GET /api/brain/status`, `GET /api/brain/diff` returning `{ diff, omittedFileCount }`, `POST /api/brain/save`) for a normal Git clone, not a Project. The Brain has no note create/rename/delete UI, so those file-op routes were removed.
- `backend/src/brain/brain-files.ts`: Brain working-tree file operations (list/read/upsert) with `requireBrainRepo()` guard (409 when absent). Path safety is shared with workspaces via `backend/src/utils/repo-files.ts` (`resolveSafeRepoFilePath`). Reads over the 1 MB cap return a bounded prefix with `truncated: true` (the agent can write notes larger than any cap directly to disk, so reads degrade to a read-only preview instead of erroring). Upsert touches disk only — never git.
- `backend/src/utils/repo-files.ts`: shared repo-relative file-path safety reused by workspace + Brain file ops — `resolveRepoFilePath()` (lexical anti-traversal, repo-root + `.git` rejection), `assertRealPathInsideRepo()` (resolves the nearest existing ancestor with `realpath` to block symlink escapes), `resolveSafeRepoFilePath()` (both combined), and the shared `MAX_TEXT_FILE_SIZE` cap.
...
- Codex App Server events are normalized into `AgentActivity` records for command execution, file changes, plan updates, image views/generations, and diagnostics. These are emitted through additive `agent_activity` WS events while `tool_use`/`tool_result` compatibility is preserved.
- Codex App Server `imageView`/`imageGeneration` items become `image_view`/`image_generation` activities. The session tracks the current turn cwd to relativize App Server absolute paths: paths inside the workspace get a `relativePath` (and `ConversationSession` derives an `imageUrl` via the shared raw-file API path builder). Image *views* outside the workspace are flagged `outsideWorkspace` and never get a preview URL. Generated images, however, are written by Codex to its own cache outside the workspace, so `ConversationSession.attachGeneratedImage` copies the saved file (≤25 MB) into the session attachments dir and serves it via the existing attachments endpoint; `finalizeTurn` awaits these copies before persisting so the served `imageUrl` lands in the saved activity (no late WS fragment). Inline base64 generation results above 256 KB are dropped to keep persisted chat payloads bounded; smaller ones render as data URLs when no saved file exists.
- `shared/workspace-files.ts` is the single source of truth for `BRAIN_WORKSPACE_ID` and `workspaceFileRawPath()` (raw-file API paths); `backend/src/utils/paths.ts`, `frontend/src/lib/brain.ts`, and `frontend/src/lib/file-preview.ts` re-export from it.
...
- Codex App Server terminations always converge to a terminal result: closing the bridge mid-turn, stopping when no interruptible turn exists, or a `turn/interrupt` rejected by Codex (stale turn id) all emit a synthetic `result` with `status: "interrupted"` instead of an `error` event, so the session finalizes (cancelled) rather than staying "running" with stale state. Synthetic interrupted results deliberately carry no session/thread id so provider continuity is never persisted from a forced termination. `ConversationSession` keeps a permanent log-only `error` listener, so an unhandled EventEmitter `error` can never crash the process when no WS client is subscribed.
- Codex App Server close/reject paths clear transient process state, including unpersisted thread ids. If an App Server process is force-closed before `providerSessionId` is persisted, the next turn starts a fresh thread instead of trying to resume a stale in-memory thread id. Stale `turn/completed` events are ignored when another active turn has already started (and are recorded as completed for dedup hygiene).
...
- `frontend/src/pages/BrainView.tsx`: two-column resizable Brain layout (`useDefaultLayout({ id: "hive-brain-v2" })`) mirroring `WorkspaceView` — a main column (left, `minSize "40%"`) with the agent chat plus a file-tab takeover via the shared `FileViewer`, and the read-only notes file tree (right, shared `FileTree`/`renderFileTreeNodes` primitives, `defaultSize "25%"`). The chat column is the shared `useConversationColumn` hook + `ConversationPane` component pointed at `workspaceId = "brain"` (so the Brain now uses the same per-session message queue as workspaces, no longer a single queue dropped on session switch). The page keeps its `handleSend` (plain), file-tab state, `renderMode` (defaults to `"rendered"` — Raw enables editing), the tree actions (`handleSelect`/`handleWriteToDisk`), and the Save flow (status badge + indicator; Save commits + pushes directly with no review modal) inline; it passes no `planActionBar` (Brain has no plan flow). The Brain has no note create/rename/delete UI — notes are authored by the agent or edited in Raw mode. Clicking a note opens it in a file tab that takes over the chat area; opening invalidates that file's content key first to defeat the `staleTime: Infinity` cache. `useBrainChatRefresh(openFile)` refreshes tree/status/open-file on agent writes. The bottom-right Sync section hosts the Save indicator + Save button.
- `frontend/src/components/diff/FileDiffCard.tsx`: shared single-file diff card (header + `@pierre/diffs` `FileDiff`) reused by `InlineDiffViewer`
...
- `frontend/src/hooks/useBrainFiles.ts`: Brain file-tree + file-content queries and the upsert mutation (invalidates the tree + git status)
- `frontend/src/hooks/useBrainGit.ts`: `useBrainStatus` (Save badge), `useBrainSave` (commit+push directly, invalidates status + parsed diff)
...
- `frontend/src/components/ui/toaster.tsx`: `HiveToast` (custom toast card: status dot, mono status word, ghost action, hover close) + `HiveToaster` (bare Sonner wrapper, bottom-left)
...
- `frontend/src/components/chat/AgentActivityList.tsx`: inline renderer/router for Codex App Server command execution, file changes, and diagnostics; plan updates are filtered into the task tracker. Image activities are delegated to `chat/ImageActivity.tsx`; the shared collapsible row primitives live in `chat/ActivityShell.tsx` (`ActivityShell` with an optional `leading` slot + `ActivityDetailChip`, reused by diagnostics)
- `frontend/src/components/chat/ImageActivity.tsx`: `image_view`/`image_generation` renderers. Each shows an inline `ImageTile` thumbnail and opens a full-screen `ImageLightbox` on click; generation rows expose the revised prompt on expand. `chat/ImageTile.tsx` is the reusable fixed-size tile (states: generating placeholder with `animate-sheen`, decode-loading, loaded/clickable, load-error, no-preview) mirroring `FileViewer`'s loading/loaded/error pattern. `chat/ImageLightbox.tsx` is the shared single-image `Dialog` lightbox (also used by `ChatMessage` attachments). Generation pending animates only while live (`showExecutingState` + non-terminal status + no resolvable image)
...
- Codex App Server command/file activities are normalized into tool-call display data, plan updates feed `TaskTracker`, and diagnostics render through `AgentActivityList`. Image view/generation activities render with an always-visible inline thumbnail tile that opens a full-screen lightbox on click; a pending generation shows an animated sheen placeholder (live only) that swaps to the real image when ready, an outside-workspace/no-preview tile otherwise, and the revised prompt on expand for generations.
...
- **Two distinct persistence levels — do not conflate them:**
  ...
  2. **Git save (commit + push):** manual, triggered by the Save button — clicking it commits + pushes the whole working tree directly (no review modal, no commit-message input). Pending changes are reviewed beforehand in the right-column "Modified" tab + per-file diff viewer. This is the backup to the remote. "Save" means publish/back up, not "save my text" (already on disk).
- Save flow: click Save -> commits + pushes immediately; the bottom-right Sync section's Save indicator shows Saving -> Saved (3s) / Push failed. No diff review modal and no commit-message input (the backend default message is used).
...
- `GET /api/brain/diff` reflects exactly what `save` would commit: tracked changes via `git diff HEAD` plus synthetic patches for untracked files (shared `getUntrackedDiff`). It returns `{ diff, omittedFileCount }`; the frontend now consumes it via `useDiff` (parsed) for the "Modified" tab + inline diff viewer. (`omittedFileCount` is no longer surfaced in the UI since the review modal was removed — a candidate for later cleanup.)
...
- `HiveMobile/Views/Chat/ChatImage.swift`: shared image stack — `ChatImageLoader` (`@Observable`, data-URL/`/api` loading with thumbnail-vs-full caching via `ImageCache`), `ChatImageTile` (fixed-size states: generating sheen, decode, loaded/tappable, error, no-preview), `ChatImageTileWithLightbox` (tracks the tile's `.global` frame and presents the lightbox via a `fullScreenCover` with the system slide suppressed — `Transaction.disablesAnimations` + `presentationBackground(.clear)` — so the lightbox owns its visuals), and `ChatImageLightbox` (custom interactive viewer: a hand-rolled zoom from the tile's rect — interpolating frame/position/corner-radius, with `scaledToFill` matching the tile crop while collapsed and the whole image once the expanded frame carries the image aspect — plus drag-to-dismiss and a `Color.black` backdrop whose opacity is coupled to the reveal + drag progress so it fades as the image nears the tile, Telegram-style. `ChatImageLoader.loadFull` seeds the cached thumbnail first so the zoom has an image on the first frame). Reused by image activities and chat message attachments
