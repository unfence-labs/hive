...
- `backend/src/brain/brain-files.ts`: Brain working-tree file operations (list/read/upsert) with `requireBrainRepo()` guard (409 when absent), anti-traversal `resolveBrainFilePath()`, and `.git` protection. Upsert touches disk only — never git.
